...
become new `/plan` tasks, not revisions.

### The cardinal rule: modify, don't rebuild

You are not `/build`. You never tear down a frame and start over. Even when
...
2. **Load the design system data.** All of these are needed for token- and
   ...
   > components. For full coverage, run `/setup-tokens` and `/setup-components`."
...
3. **Get the target frame.**
   ...
   > "Select the frame you want to revise in Figma, then run `/revise` again."

4. **Get the feedback source.** This is the first real question — ask it clearly.

   If the user already provided feedback inline (e.g., `/revise make the
...
> | 5 | "Can we add a filter bar?" | Scope change | New feature — needs `/plan` | Deferred |
...
If the user runs `/revise` again on an already-revised frame, increment:
...
> - "Can we add a filter bar?" — This is a new feature. Run `/plan` to
>   plan it, then `/build` to add it.
>
...
> better handled by `/plan` + `/build` for that section. Want me to:
>
> A) Do my best with surgical changes (may feel patchy)
> B) Flag this for a `/plan` redesign of just this section
...
> via `/plan`). 1 is preference-based (needs your decision on card border
