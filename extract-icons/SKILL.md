...
description: |
  ...
  Makes icons instantly resolvable in plan-design, plan-component, and build skills.
allowed-tools:
...
When `/plan-design` says "use a search icon," it needs to resolve "search" into
...
> - `/plan-design`: 'use a search icon' resolves to a key instantly
> - `/build-design`: swaps icons into slots without searching
...
>
> - Run `/plan-design` or `/plan-component` — icon suggestions will be automatic
> - Run `/build-design` — icon swaps will resolve without searching Figma
> - Run `/extract-components` — if you haven't already, this catalogs the full
>   component library (icons are the lightweight complement)
> - Run `/extract-relationships` — to map which components use which icons
>   in their swap slots"
