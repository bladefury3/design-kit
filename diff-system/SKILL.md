...
>
> - `design-system/tokens.json` — run `/extract-tokens`
> - `design-system/components/index.json` — run `/extract-components`
> - `design-system/relationships.json` — run `/extract-relationships`
>
> Once extracted, run `/diff-system` anytime to track changes."
...
`/extract-tokens` and `/extract-components` use, but optimized for comparison
...
>
> **A) Full re-extract** — Run `/extract-tokens` + `/extract-components` + `/extract-relationships`
...
> "System looks stable. No re-extraction needed right now. Run `/diff-system`
