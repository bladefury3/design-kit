...
description: |
  ...
  composition patterns. Use after setup-components to complete the design system graph.
allowed-tools:
...
1. Confirm Figma is connected.
   ...
   **Note on library components**: Unlike variables, the Figma Plugin API has NO
   ...
   the user should run `/setup-components` first, which handles the multi-step
...
>
> Or run `/setup-tokens` and `/setup-components` first — they'll create the
> JSONs I need."
