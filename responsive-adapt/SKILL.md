...
description: |
  ...
  Outputs breakpoint plans in plans/ that build-design executes.
allowed-tools:
...
**You produce breakpoint plans** in `plans/` that `/build-design` executes
...
### Responsive-specific metadata (beyond standard plan-design)
...
`/build-design` uses the `layout` tree; the metadata documents WHY.
...
during build. Flag any path-style keys: "Run `/extract-tokens` to refresh."
...
### Frame placement (horizontal row, 100px gap)
...
Follow `/build-design` mechanics for each breakpoint plan:
